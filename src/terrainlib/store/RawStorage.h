...
public:
    ...
    RawStorage(Layout<Key> layout, std::unique_ptr<Codec<NodeData>> codec)
        : _layout(std::move(layout)), _codec(std::move(codec)) {}
    ...
    std::expected<NodeData, LoadError<Key>> load(const Key &key) const {
        ...
        const auto result = _codec->read(_layout.node_path(key));
        if (!result.has_value()) {
    ...
    std::expected<void, SaveError<Key>> save(const Key &key, const NodeData &data) const {
        ...
        const auto result = _codec->write(_layout.node_path(key), data);
        if (!result.has_value()) {
    ...
    std::expected<std::vector<std::filesystem::path>, InvalidKey<Key>> paths(
        ...
        return _codec->paths(_layout.node_path(key));
    }
    ...
    std::expected<void, CopyError<Key>> copy_from(
        ...
        if (_codec->paths(probe) != source._codec->paths(probe)) {
            const auto loaded = source._codec->read(source._layout.node_path(key));
            if (!loaded.has_value()) {
...
private:
    Layout<Key> _layout;
    std::unique_ptr<Codec<NodeData>> _codec;
};
