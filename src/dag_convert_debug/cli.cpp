...
Args cli::parse(int argc, const char *const *argv) {
    ...
    app.add_option("--input", args.input_path, "Path to a DAG storage folder, or a single node file")
        ->required()
        ->check(CLI::ExistingPath);

    app.add_option("--output", args.output_path, "Path to write debug meshes to: a folder if --input is a folder, or a mesh file if --input is a file")
        ->required();
