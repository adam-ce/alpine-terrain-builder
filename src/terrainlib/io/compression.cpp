...
namespace {
    ...
    Expected<void> validate_algorithms(const CompressionAlgorithm compression_algorithm, const ChecksumAlgorithm checksum_algorithm)
    {
        ...
        if (!no_compression && !zstd_with_checksum) {
            return Error::fail(Error::Code::InvalidInput, "invalid compression and checksum algorithm combination");
        }
    ...
    const std::size_t max_decompressed_size)
{
    ...
    if (checksum_algorithm == ChecksumAlgorithm::Crc32c && (checksum.size() != 8 || !std::all_of(checksum.begin(), checksum.end(), [](const char character) {
        ...
        return Error::fail(Error::Code::CorruptData, "invalid CRC32C checksum encoding");
    }
    ...
    if (compression_algorithm == CompressionAlgorithm::None) {
        ...
        if (frame_header.checksumFlag == 0) {
            return Error::fail(Error::Code::CorruptData, "Zstandard frame does not contain the required checksum");
        }
    ...
    if (checksum_algorithm == ChecksumAlgorithm::Crc32c && crc32c_checksum(uncompressed_data) != checksum) {
        return Error::fail(Error::Code::CorruptData, "CRC32C checksum mismatch");
    }
