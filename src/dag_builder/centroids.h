...
inline std::vector<uint32_t> find_clusters_inside_any_bounds(
    const std::span<const glm::dvec3> centroids,
    const std::span<const radix::geometry::Aabb3d> bounds) {
    return detail::find_clusters_by_centroid_predicate(
        centroids,
        [bounds](const glm::dvec3 &centroid) {
            return detail::contained_in_any_bounds(centroid, bounds);
        });
}

inline std::vector<uint32_t> find_clusters_inside_all_bounds(
    const std::span<const glm::dvec3> centroids,
    const std::span<const radix::geometry::Aabb3d> bounds) {
    return detail::find_clusters_by_centroid_predicate(
        centroids,
        [bounds](const glm::dvec3 &centroid) {
            return detail::contained_in_all_bounds(centroid, bounds);
        });
}
...
inline std::vector<uint32_t> find_clusters_inside_bounds(
    ...
    return find_clusters_inside_bounds(centroids, bounds);
}
...
inline std::vector<uint32_t> find_clusters_inside_any_bounds(
    ...
    const std::span<const radix::geometry::Aabb3d> bounds) {
    const auto centroids = compute_cluster_centroids(clustering);
    return find_clusters_inside_any_bounds(centroids, bounds);
}
...
inline std::vector<uint32_t> find_clusters_inside_all_bounds(
    ...
    const std::span<const radix::geometry::Aabb3d> bounds) {
    const auto centroids = compute_cluster_centroids(clustering);
    return find_clusters_inside_all_bounds(centroids, bounds);
}

inline std::vector<uint32_t> find_clusters_outside_bounds(
    const std::span<const glm::dvec3> centroids,
    const radix::geometry::Aabb3d &bounds) {
    return detail::find_clusters_by_centroid_predicate(
        centroids,
...
inline std::vector<uint32_t> find_clusters_outside_any_bounds(
    const std::span<const glm::dvec3> centroids,
    const std::span<const radix::geometry::Aabb3d> bounds) {
    return detail::find_clusters_by_centroid_predicate(
        centroids,
...
inline std::vector<uint32_t> find_clusters_outside_all_bounds(
    const std::span<const glm::dvec3> centroids,
    const std::span<const radix::geometry::Aabb3d> bounds) {
    return detail::find_clusters_by_centroid_predicate(
        centroids,
...
inline std::vector<uint32_t> find_clusters_outside_bounds(
    ...
    const radix::geometry::Aabb3d &bounds) {
    const auto centroids = compute_cluster_centroids(clustering);
    return find_clusters_outside_bounds(centroids, bounds);
}
...
inline std::vector<uint32_t> find_clusters_outside_all_bounds(
    ...
    const std::span<const radix::geometry::Aabb3d> bounds) {
    const auto centroids = compute_cluster_centroids(clustering);
    return find_clusters_outside_all_bounds(centroids, bounds);
}
