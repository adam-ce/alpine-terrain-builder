...
using MetadataSchema = io::envelope::PayloadSchema<
    "dag.NodeMetadata",
    io::envelope::Version<1, v1::NodeMetadata>>;
using ClusteringSchema = io::envelope::PayloadSchema<
    "dag.Clustering",
    io::envelope::Version<1, v1::Clustering>>;
using NodeMetadata = MetadataSchema::latest_type;
using Clustering = ClusteringSchema::latest_type;

inline v1::OctreeId encode_id(const octree::Id &id)
{
    return {.level = id.level(), .index = id.index_on_level()};
}

inline std::expected<octree::Id, std::string> decode_id(const v1::OctreeId &id)
{
    auto result = octree::Id::try_make(
        static_cast<octree::Id::Level>(id.level),
        static_cast<octree::Id::Index>(id.index));
    if (!result) {
...
inline std::expected<Clustering, std::string> encode_clustering(
...
inline bool count_fits(const std::uint64_t count, const std::size_t element_size)
{
    return count <= std::numeric_limits<std::size_t>::max() / element_size
        && count * element_size <= io::envelope::default_max_decompressed_size;
}
...
inline std::expected<void, std::string> validate(const Clustering &clustering)
{
    ...
    std::size_t decoded_size =
        static_cast<std::size_t>(clustering.vertex_count) * sizeof(glm::dvec3);
    for (const v1::Cluster &cluster : clustering.clusters) {
        if (!count_fits(cluster.triangle_count, sizeof(glm::uvec3))
            || !count_fits(cluster.vertex_count, sizeof(std::uint32_t))
        ...
        const std::size_t cluster_size =
            static_cast<std::size_t>(cluster.triangle_count) * sizeof(glm::uvec3)
            + static_cast<std::size_t>(cluster.vertex_count) * sizeof(std::uint32_t)
            + static_cast<std::size_t>(cluster.uv_count) * sizeof(glm::dvec2);
        ...
        if ((cluster.triangle_count != 0 && cluster.encoded_triangles.empty())
            || (cluster.vertex_count != 0 && cluster.encoded_vertex_indices.empty())
        ...
        if (cluster.texture_id != (std::numeric_limits<std::uint32_t>::max)()
            && cluster.texture_id >= clustering.textures.size()) {
...
inline std::expected<std::vector<glm::uvec3>, std::string> decode_triangles(
    const std::uint64_t count,
    const std::vector<std::uint8_t> &encoded)
{
...
inline std::expected<::Clustering, std::string> decode_clustering(Clustering clustering)
{
    ...
    auto positions = decode_vertex_buffer<glm::dvec3>(
        clustering.vertex_count,
        clustering.encoded_positions,
        "DAG positions");
    if (!positions) {
    ...
    for (const v1::Cluster &encoded : clustering.clusters) {
        ...
        auto vertex_indices = decode_vertex_buffer<std::uint32_t>(
            encoded.vertex_count,
            encoded.encoded_vertex_indices,
            "DAG vertex indices");
        auto uvs = decode_vertex_buffer<glm::dvec2>(
            encoded.uv_count,
            encoded.encoded_uvs,
            "DAG UVs");
        if (!triangles) {
        ...
        for (const glm::uvec3 triangle : *triangles) {
            if (glm::any(glm::greaterThanEqual(
                    triangle,
                    glm::uvec3(static_cast<std::uint32_t>(vertex_indices->size()))))) {
        ...
        result.clusters.push_back({
            ...
            .uvs = std::move(*uvs),
            .texture_id = encoded.texture_id == (std::numeric_limits<std::uint32_t>::max)()
                ? std::nullopt
                : std::optional<std::uint32_t>{encoded.texture_id},
