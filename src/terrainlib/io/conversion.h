...
struct PixelTraits {
    ...
    static channel_type &channel(Pixel &pixel, const int)
    {
        return pixel;
    }

    static const channel_type &channel(const Pixel &pixel, const int)
    {
        return pixel;
    }
};
...
struct PixelTraits<glm::vec<Length, Channel, Qualifier>> {
    ...
    static constexpr bool supported = channels >= 2 && channels <= 4
        && cv_depth<channel_type> >= 0;

    static channel_type &channel(glm::vec<Length, Channel, Qualifier> &pixel, const int index)
    {
        return pixel[static_cast<glm::length_t>(index)];
    }

    static const channel_type &channel(
        const glm::vec<Length, Channel, Qualifier> &pixel,
        const int index)
    {
        return pixel[static_cast<glm::length_t>(index)];
    }
...
template <typename Pixel>
inline constexpr int cv_type = Traits<Pixel>::supported
    ? CV_MAKETYPE(cv_depth<typename Traits<Pixel>::channel_type>, Traits<Pixel>::channels)
    : -1;

template <typename Pixel>
inline constexpr bool has_packed_layout = Traits<Pixel>::supported
    && std::is_trivially_copyable_v<Pixel>
    && sizeof(Pixel) == sizeof(typename Traits<Pixel>::channel_type) * Traits<Pixel>::channels;
...
void copy_mat_to_raster(const cv::Mat &source, radix::Raster<Pixel> &destination)
{
    ...
    if constexpr (has_packed_layout<Pixel>) {
        for (int row = 0; row < source.rows; ++row) {
            std::memcpy(
                destination.data() + static_cast<std::size_t>(row) * source.cols,
                source.ptr(row),
                row_bytes);
        }
    } else {
        for (int row = 0; row < source.rows; ++row) {
            ...
            for (int column = 0; column < source.cols; ++column) {
                ...
                for (int channel = 0; channel < PixelInfo::channels; ++channel) {
                    PixelInfo::channel(pixel, channel) =
                        source_row[column * PixelInfo::channels + channel];
                }
            }
        }
    }
...
void copy_raster_to_mat(const radix::Raster<Pixel> &source, cv::Mat &destination)
{
    ...
    if constexpr (has_packed_layout<Pixel>) {
        for (int row = 0; row < destination.rows; ++row) {
            std::memcpy(
                destination.ptr(row),
                source.data() + static_cast<std::size_t>(row) * destination.cols,
                row_bytes);
        }
    } else {
        for (int row = 0; row < destination.rows; ++row) {
            ...
            for (int column = 0; column < destination.cols; ++column) {
                ...
                for (int channel = 0; channel < PixelInfo::channels; ++channel) {
                    destination_row[column * PixelInfo::channels + channel] =
                        PixelInfo::channel(pixel, channel);
                }
            }
        }
