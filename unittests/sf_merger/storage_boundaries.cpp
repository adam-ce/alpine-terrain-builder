...
TEST_CASE(
    "SF merge retains an invalid output index for diagnosis",
    "[sfmerger][sf][validation]") {
...
TEST_CASE("SF merge hard-links unchanged nodes and writes changed nodes", "[sfmerger][sf][copy]") {
    ...
    SECTION("unchanged") {
        ...
        CHECK(std::filesystem::equivalent(
            left.paths(root)->front(),
            output.paths(root)->front()));
    }
