...
inline std::string describe_error(const Error &error)
{
    return "envelope error " + std::to_string(static_cast<unsigned>(error.code));
}
...
std::expected<typename Schema::latest_type, FileError> read_from_path(
    ...
    const std::size_t max_decompressed_size = default_max_decompressed_size)
{
...
template <typename Schema>
std::expected<void, FileError> write_to_path(
