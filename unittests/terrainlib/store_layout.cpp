...
TEST_CASE("extensionless octree mappings validate complete paths", "[store][layout]") {
    ...
    CHECK_FALSE(flat.node_path_to_key(store::NodePath("1-2.terrain")).has_value());
    ...
    CHECK_FALSE(coordinates.node_path_to_key(store::NodePath("1/0/0/0.terrain")).has_value());
...
TEST_CASE("runtime mesh codecs own their filename endings", "[store][layout][codec]") {
    ...
    CHECK(terrain.paths(path) == std::vector<std::filesystem::path>{"12/34/56/78.terrain"});
