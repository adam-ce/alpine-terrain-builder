    ...
    namespace format {
        ...
        inline Expected<Clustering> encode_clustering(const ::Clustering& clustering)
        {
            ...
            try {
                ...
                return Error::fail(classify_texture_error(error, Error::Code::InvalidInput), "could not encode DAG texture: " + error.msg);
            }
            return result;
        }
        ...
        template <typename T>
        inline Expected<std::vector<T>> decode_vertex_buffer(const std::uint64_t count, const std::vector<std::uint8_t>& encoded, const std::string_view label)
        {
        ...
        inline Expected<::Clustering> decode_clustering(Clustering clustering)
        {
            ...
            try {
                ...
                return Error::fail(classify_texture_error(error, Error::Code::CorruptData), "could not decode DAG texture: " + error.msg);
            }
...
Expected<dag::ClusterBatch> ClusterBatch::read(const std::filesystem::path& node_path) const
{
    ...
    if (auto valid = validate_batch(batch, Error::Code::CorruptData); !valid) {
        return Error::propagate(std::move(valid), "validate DAG files \"" + node_paths[0].string() + "\" and \"" + node_paths[1].string() + "\"");
    }
    return batch;
}
...
std::vector<std::filesystem::path> Metadata::paths(const std::filesystem::path& node_path) const { return { add_extension(node_path, ".dagmeta") }; }

Expected<dag::NodeMetadata> Metadata::read(const std::filesystem::path& node_path) const { return read_metadata(paths(node_path).front()); }
