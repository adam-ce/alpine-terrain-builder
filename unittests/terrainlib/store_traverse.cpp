...
TEMPLATE_TEST_CASE(
    "shared hierarchy traversal preserves trait child order",
    "[store][traverse]",
    octree::StoreTraits,
    ...
    REQUIRE(store::traverse(
                index,
                [&](const Key &key, const store::NodeStatus) { depth_first.push_back(key); })
                .has_value());
    ...
    REQUIRE(store::traverse(
                index,
                [&](const Key &key, const store::NodeStatus) { explicit_root.push_back(key); },
                store::AlwaysRefine{},
                first)
                .has_value());
...
TEST_CASE("shared traversal rejects an invalid explicit root", "[store][traverse][raster]") {
    ...
    const auto result = store::traverse(
        index,
        [](const Key &, const store::NodeStatus) {},
        store::AlwaysRefine{},
        invalid);
