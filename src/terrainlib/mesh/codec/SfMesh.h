...
public:
    ...
    std::expected<mesh::Simple, store::CodecError> read(
        const store::NodePath &node_path) const override
    {
        auto payload = ::io::envelope::read_from_path<mesh::sf::Schema>(
            paths(node_path).front());
        if (!payload) {
            const bool missing = std::holds_alternative<::io::Error>(payload.error())
                && std::get<::io::Error>(payload.error()) == ::io::Error(::io::Error::OpenFile);
    ...
    std::expected<void, store::CodecError> write(
        ...
        const mesh::EncodeOptions options) const
    {
        ...
        auto result = ::io::envelope::write_to_path<mesh::sf::Schema>(
            *payload,
            paths(node_path).front());
        if (!result) {
            return std::unexpected(store::CodecError{
