...
{
    if (payload.vertex_count
        > ::io::envelope::default_max_decompressed_size / sizeof(mesh::Simple::Position)) {
        return std::unexpected("SF mesh vertex count exceeds the allocation limit");
    }
    if (payload.face_count
        > ::io::envelope::default_max_decompressed_size / sizeof(mesh::Simple::Triangle)) {
        return std::unexpected("SF mesh face count exceeds the allocation limit");
    }
    if (payload.vertex_count == 0
        && (!payload.positions.empty() || !payload.uvs.empty())) {
...
inline std::expected<Payload, mesh::EncodeError> encode_payload(
    const mesh::Simple &mesh,
    const mesh::EncodeOptions options = {})
{
